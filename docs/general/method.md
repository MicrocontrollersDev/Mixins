...
In the case of there existing multiple methods with the same name, use McDev to autocomplete the correct one with the descriptor (i.e., `foo(I)V`).

If you actually do want to target all methods, you can simply use a wildcard, `*`. For example, `foo*` will target all "foo" methods. You can also simply target a wildcard itself, matching with every method in the class.
